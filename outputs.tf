...
output "manager_instances" {
  ...
  value = join("\n", [for instance in google_compute_instance.swarm_manager : 
    "Name: ${instance.name}, Internal IP: ${instance.network_interface[0].network_ip}, External IP: ${instance.network_interface[0].access_config[0].nat_ip}, Zone: ${instance.zone}"
  ])
}
